...
#cd8tspc <- subset(cd8t, subset = cell_names %in% cd8tspc_inf$cell_names)
## Fig.4D
...
tumorseu <- subset(seur, sampleType == "PTC")
tumorseu$cell_lineages <- as.character(tumorseu$cell_lineages)
tumorseu$cell_lineages[str_detect(tumorseu$clusters_anno, "^DC_")] <- "DC"

AIThighdif <- readRDS("./RData/05_AIThighdifcell.rds")
tumorseu$TLScluster <- ifelse(tumorseu$clusters_anno %in% AIThighdif, "TLS", "NoTLS")

tumorseu <- subset(tumorseu, 
                  cell_lineages %in% c("CD4T", "CD8T", "ucT/NK",
                                                "B/plasma", "DC", "Myeloid"))
tumorseu$TLScluster <- paste0(tumorseu$cell_lineages, "_", tumorseu$TLScluster)
p <- DotPlot(tumorseu, features = TLSsig, 
        group.by = "TLScluster") + 
